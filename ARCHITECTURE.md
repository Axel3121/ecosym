...
- **Input** — whatever Rook learns from: conversation with the user, and later
  the sources they connect.
- **Memory** — the durable store of what Rook knows about its user, their
...
- **Action** — the effects Rook can have outside itself. Consequential ones
  require the user's approval.
- **Surface** — how the user reaches Rook. Deliberately thin; no logic lives
  here.
...
- Rook is a single-user system. Do not generalise for tenants or teams.
- Prefer the smallest thing that works. Complexity has to be earned by a real
  problem that has already appeared.

## Not decided

Language, runtime, storage, model provider, and how Rook is deployed and
