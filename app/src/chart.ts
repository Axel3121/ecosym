...
function plateRect(s: Settlement) {
  ...
  const p = PLATES[key]!;
  const w = s.radius * 2.2, h = w / p.aspect;
...
export class Chart {
  ...
  constructor(private canvas: HTMLCanvasElement, public camera: Camera = { x: 760, y: 470, zoom: 0.84 }) {
  ...
  draw(scene: Scene, walkers: Walker[], selected: Hit | null) {
    ...
    // plates (settlement level), crossfaded over the world
    if (plateAlpha > 0) {
      for (const s of scene.settlements) {
        if (s.epistemic !== "observed") continue;
        ...
        ctx.drawImage(im, a.x, a.y, r.w * z, r.h * z);
        ctx.globalAlpha = 1;
      }
      const cap = this.im("/art/capital.png");
      if (cap) {
        const w = CAPITAL.r * 2.3;
        const a = this.toScreen(CAPITAL.x - w / 2, CAPITAL.y - w / 2);
        ctx.globalAlpha = plateAlpha; ctx.drawImage(cap, a.x, a.y, w * z, w * z); ctx.globalAlpha = 1;
      }
    }
  ...
  private label(x: number, y: number, text: string, alpha: number) {
    const { ctx } = this;
    ctx.save(); ctx.globalAlpha = alpha; ctx.font = `700 15px "Pixelify Sans", monospace`; ctx.textAlign = "center";
    const w = ctx.measureText(text).width + 14;
    ctx.fillStyle = "rgba(30,24,16,0.75)"; ctx.fillRect(x - w / 2, y - 13, w, 20);
    ctx.fillStyle = "#f3e7c8"; ctx.fillText(text, x, y + 2); ctx.restore();
  }
