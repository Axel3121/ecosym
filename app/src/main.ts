...
function select(h: Hit | null) {
  ...
  sheet.querySelector("form")?.addEventListener("submit", (ev) => {
    ev.preventDefault();
    const ta = sheet.querySelector("textarea")!;
    const note = sheet.querySelector<HTMLElement>(".petition-note")!;
    note.textContent = `Not sent: the petition path is not built (no identity, no authority binding). "${ta.value.slice(0, 60)}" would have gone to ${h.label}.`;
  });
}
...
function sheetFor(h: Hit): string {
  ...
  switch (h.kind) {
    case "capital": {
      ...
      return `${close}<p class="kind">the world's headquarters</p><h2>Capital</h2><h3>Halls</h3><ul class="halls">${halls}</ul><h3>Before the council</h3><ul>${scene.capital.matters.map((m) => `<li>${m.summary} <span class="muted">${ago(m.raisedAt)}</span></li>`).join("")}</ul><p class="muted">Detail lives where the work lives. Click a hall to go there.</p>${syn}`;
    }
    case "settlement": {
      if (!s) return close;
      ...
      return `${close}<p class="kind">civilization · ${s.domain}</p><h2>${s.name}</h2><p>last seen <span class="muted">${ago(s.lastSeen!)}</span> · ${s.inhabitants.length} at work · ${s.traces.length} recent traces</p><h3>Seat</h3><p class="muted">${s.seatName} — click it inside the walls to address it.</p><h3>Working now</h3><ul>${s.inhabitants.map((i) => `<li>${"— ".repeat(i.depth)}${i.label}${i.tool ? ` <span class="muted">(${i.tool})</span>` : ""}</li>`).join("") || '<li class="muted">nobody</li>'}</ul>${syn}`;
    }
    case "seat": {
      if (!s) return close;
    ...
    case "inhabitant": {
      ...
      if (inh) {
        ...
        return `${close}<p class="kind">transient inhabitant · running now</p><h2>${inh.label}</h2><ul><li>tool: ${inh.tool ?? "—"}</li><li>depth: ${inh.depth}${inh.parentRunId ? ` (under ${s.inhabitants.find((p) => p.runId === inh.parentRunId)?.label ?? inh.parentRunId})` : ""}</li></ul>${children.length ? `<h3>Delegated</h3><ul>${children.map((c) => `<li>${c.label}</li>`).join("")}</ul>` : ""}<p class="muted">Leaves when the work leaves. No standing office.</p>${syn}`;
      }
...
// test hook
(window as unknown as { __ecosym: unknown }).__ecosym = { scene, chart, select, flyTo };
